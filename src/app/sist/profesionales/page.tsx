...
export default function MisPacientesPage() {
  ...
  const misPacientes = mockPacientes.filter((paciente) => {
    ...
    const tieneTurno = mockTurnos.some(
      (turno) =>
        turno.pacienteId === paciente.id && turno.profesionalId === user?.id
    );
  ...
  return (
    <div className="space-y-6 pt-16 flex-1">
      {/* Header */}
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Mis Pacientes</h1>
          <p className="text-gray-600">
            Gestiona los pacientes asignados a ti ({pacientesFiltrados.length}{" "}
            pacientes)
          </p>
        </div>
      </div>
                ...
                <div className="flex justify-between items-start">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
                      ...
                      <div>
                        ...
                        {paciente.proximoTurno
                          ? new Date(paciente.proximoTurno).toLocaleDateString()
                          : "Sin turnos"}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 ml-4">
                    ...
                    <Button
                      variant="default"
                      size="sm"
                      onClick={() => handleNuevaConsulta(paciente)}
                    >
