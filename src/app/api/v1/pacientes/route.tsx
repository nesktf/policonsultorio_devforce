...
export async function POST(request: Request) {
  ...
  if (typeof fecha_nacimiento !== 'string' || fecha_nacimiento.trim().length === 0) {
    return NextResponse.json(
      { error: 'Fecha Nacimiento es requerida y debe ser un string con formato YYYY-MM-DD.' },
      { status: 400 },
    );
  }
  ...
  if (Number.isNaN(fechaNacimientoDate.getTime())) {
    return NextResponse.json(
      { error: 'Fecha Nacimiento inválida. Utilice el formato YYYY-MM-DD.' },
      { status: 400 },
    );
  }
  ...
  if (obraSocialId !== null && !isNonEmptyString(num_obra_social)) {
    return NextResponse.json(
      { error: 'Nro OOSS es requerido cuando se indica una obra social.' },
      { status: 400 },
    );
  }
