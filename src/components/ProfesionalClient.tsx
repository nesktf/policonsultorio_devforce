...
import {
  ...
  type DatosProfesionalFormulario,
} from "../components/RegistrarProfesionalModal";
...
export function ProfesionalClient({
  ...
  const queryProfesionales = async (
    nombre?: string,
    especialidad?: string,
    obraSocialId?: string
  ) => {
    ...
    try {
      const response = await fetch(
        buildGETQuery(nombre, especialidad, obraSocialId),
        {
          method: "GET",
        }
      );
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const error = new Error(
          errorData.error || "Error al registrar profesional"
        );
        throw error;
      }

      // Recargar la lista después de crear
      await queryProfesionales(
        searchTerm,
  ...
  return (
    <div className="p-6 bg-[#E4F1F9] min-h-screen pt-16">
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-sm border-b-2 border-[#AFE1EA] bg-[#E4F1F9]">
              <tr>
                ...
                <th
                  scope="col"
                  className="px-6 py-4 font-bold text-[#0AA2C7] text-center"
                >
                  Turnos
                </th>
                <th
              ...
              {loading ? (
                ...
                <tr>
                  <td
                    ...
                    className="px-6 py-8 text-center text-gray-500"
                  >
                    No se encontraron profesionales con los criterios
                    seleccionados
                  </td>
                </tr>
              ) : (
