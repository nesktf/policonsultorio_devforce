...
export function NuevaConsultaDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Crear fecha local y ajustar a UTC para Argentina (GMT-3)
      const fechaHoraLocal = `${formData.fecha}T${formData.hora}:00`;
      const fechaLocal = new Date(fechaHoraLocal);
      // Sumar 3 horas para compensar la zona horaria de Argentina (GMT-3)
      const fechaUTC = new Date(fechaLocal.getTime() + (3 * 60 * 60 * 1000));
